...
\texttt{SVAR()}.\footnote{The exhibition is confined to the function
  \texttt{SVAR()}. Incidentally, with function \texttt{SVAR2()} a SVAR
  model can estimated too, thereby a scoring algorithm as suggested by
  \citeasnoun{AMI97} is utilised.} Its arguments are shown in the
...
\texttt{SVAR2()} a scoring algorithm is utilised for determining the
