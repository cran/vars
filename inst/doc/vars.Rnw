...
\end{equation}
The $\Gamma_i$ matrices contain the cumulative long-run impacts, hence this VECM specification is signified by longrun form. The other specification is given as follows:
%
\begin{equation}
\label{eq32}
...
\par
With function \texttt{vec2var()} a VECM (\emph{i.e} an object of formal class \texttt{ca.jo}, generated by \texttt{ca.jo()}) is transormed into its level VAR representation. For illustration purposes, an example taken from \citeasnoun{JOH1990} is used in the following.
%
...
@ 
%
One can conclude that two cointegration relationships do exist. The cointegrating rank is needed for the above transformation with function \texttt{vec2var}. Given the object \texttt{sjf.vecm} of class \texttt{ca.jo} with $r = 2$, one can now swiftly proceed:
